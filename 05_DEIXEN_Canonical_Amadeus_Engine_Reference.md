...
# DEIXEN — Canonical Amadeus Engine Reference
...
sequencing decision owned by `07_DEIXEN_Canonical_Decisions_and_Current_State.md`,
...
- **Implementation truth** — what the supplied DEIXEN repository
...
| `FXP` / `FXB` | Fare pricing implemented by the supplied engine. The engine recognizes `FXP`/`FXB` and has no `FXX` handler. **Separate Amadeus evidence has since resolved `FXX` as a real Amadeus command; therefore the absence of an `FXX` handler here is an implementation-scope fact, not evidence that `FXX` is invalid in real Amadeus.** | **Code-verified** |
...
**Pre-Opus domain-truth quarantine:** the Pre-Opus Amadeus evidence package identifies a material domain-truth vs. implementation-truth reconciliation issue for `QE`/`QN`/`QD`. The descriptions in this table remain **implementation truth only** until source-level reconciliation is completed; they must not be promoted as real-world Amadeus behavior merely because they are code-verified.
...
workstream (see `07_DEIXEN_Canonical_Decisions_and_Current_State.md`
