...
in `04_DEIXEN_Canonical_Design_System.md` for the one place this matters
in practice.
...
**Phase:** Pre-Opus Foundation Preparation / Foundation Hardening. The active
phase is the controlled preparation of the synchronized project Corpus for
independent Opus review. Design Execution remains a downstream phase and is
not the current execution objective. Design Positioning remains closed and
unaffected. See `04_DEIXEN_Canonical_Design_System.md`.

**Current objective:** complete the Pre-Opus Foundation Preparation sequence,
including Amadeus evidence stabilization, operating-layer review,
authority/dependency reconciliation, current-state synchronization,
cross-Corpus synchronization, and the final independent-review handoff.
No implementation or design execution is authorized merely by this status.

**Implementation baseline reference (not the current execution target):** a "Rebound Baseline — Replacement Controlled
...
instead. The current Pre-Opus phase does not authorize implementation work against either baseline. Full evidence in
...
### Amadeus Engine — owned in detail by `05_DEIXEN_Canonical_Amadeus_Engine_Reference.md`
- The 37-command engine is code-verified and current — see
  `05_DEIXEN_Canonical_Amadeus_Engine_Reference.md` for exactly what
...
- **Pre-Opus Amadeus evidence state:** the completed Amadeus evidence package is now the controlled pre-Opus handoff for Amadeus knowledge. It does not supersede this ledger's decision authority, does not authorize implementation, and is not yet final learner-facing/Engine-authoritative knowledge. Its unified carry-forward register is the authoritative location for unresolved Amadeus evidence questions during the pre-Opus phase. Material carry-forwards include the `QE`/`QN`/`QD` implementation reconciliation, SSR/seat association workflow depth, voiding, the formal `FXX` curriculum-documentation retraction, and the qualified `B5` status.
...
`05_DEIXEN_Canonical_Amadeus_Engine_Reference.md` — need this same
...
- Manus Collaboration Protocol (Decision 10A) — **Closed, Approved Operating
  Rule.** The currently supplied unified AI Working Rules artifact is a
  **PROPOSED UNIFIED DRAFT — NOT YET CANONICAL**; it may inform this rule but
  does not override the decision ledger or create new authority until separately
  accepted.
- Cross-Review Arbitration Outcome (Decision 10) — **Closed, Approved
  ...
  Design section above and `04_DEIXEN_Canonical_Design_System.md`).
