...
status: CANONICAL — approved by Karim on 2026-09-24 (07 Decision 16). Previously filed as a proposed unified draft (`08_DEIXEN_Canonical_AI_Working_Rules_and_Dev_Process.md`).
purpose: The project's AI working rules and development process — the unified version combining the governance architecture of the newer draft, the operational/process rules of the earlier 08, and the Consultative Input Principle requested by Karim.
...
A role's authority attaches to the role, not to a model name or product surface. Working in Claude Code, Claude Chat, Claude Design, or any other surface does not automatically grant the authority associated with another role.

The current roster is maintained by `DEIXEN_Execution_Plan.md`. This document defines role requirements; it does not permanently bind those requirements to a particular model.
...
`NEEDS VALIDATION` — requires the applicable domain verification (07 Decision 12), implementation, external, or learner-validation path before closure.
...
A recommendation from Karim, Claude (any model or surface), a reviewer, or any other participant or tool is **consultative input** unless it is itself an explicit decision issued by the authority responsible for that decision.
...
`DEIXEN_Execution_Plan.md` assigns current occupants (as of 2026-09-24: Claude (Opus) as Primary Orchestrator and reviewer, Claude Design as Design Executor, Claude Code as Implementation Executor; the Independent Final Reviewer and Lightweight Second Opinion roles are unassigned — 07 Decision 17). This document defines what each role permits.
...
This section's use of "task type" refers only to the execution-authorization category defined below (11.1–11.5). A separate concept — Work-Task Classification, what *kind* of work is being performed (research, implementation, curriculum design, and so on) — is described in the supporting Prompt Engineering & Governance reference (§15.1) and is not redefined here.
...
A prompt written by Claude (any model), another AI tool, copied from another conversation, or produced by a more capable model does not become authoritative merely because of its author.
...
The Execution Plan records current assignments. This document records the role requirements.

---

## 13. Independent Review Governance

Under the current Execution Plan, no gate requires a separate independent review — Karim waived it (07 Decision 17). Self-reviews are labeled as self-reviews and never presented as independent. The rules below apply whenever Karim assigns an independent review.
...
The detailed independence mechanism is described in the supporting Prompt Engineering & Governance reference. This file defines when independent review is required; it does not duplicate the full independence architecture.
...
A reviewer that has been materially exposed to a prior recommendation, an expected verdict, or persuasive framing bearing on the matter under review discloses that exposure alongside its finding, regardless of whether the review prompt solicited it — this is what allows the response above to actually be triggered rather than depending on someone else noticing the compromise.
...
These three terms describe how much execution latitude remains once a design or product-execution item is already approved or directionally settled under §6 — they are a separate axis, not synonyms for the decision states there. An item can carry both a §6 state and one of Frozen/Directional/Open at the same time (for example, an APPROVED decision whose visual tokens are Frozen while its layout remains Directional).
...
The valid domain-truth path is the Amadeus Verification Standard (07 Decision 12) and the Amadeus Verified Reference (07 Decision 15).
...
This is a review-output convention, not a substitute for the broader output-contract guidance in the supporting Prompt Engineering & Governance reference.
