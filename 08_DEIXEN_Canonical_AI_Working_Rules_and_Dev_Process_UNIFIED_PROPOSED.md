...
These eight states classify a decision's workflow status specifically. They are a separate axis from, and are not a replacement for, the DEIXEN Operating Constitution's own claim-classification system, nor for the claim-level epistemic scale (fact/observed through hypothesis/unknown) defined in Prompt Engineering & Governance §29 — a single item may carry a status from each without the terms being synonyms.
...
This section's use of "task type" refers only to the execution-authorization category defined below (11.1–11.5). A separate concept — Work-Task Classification, what *kind* of work is being performed (research, implementation, curriculum design, and so on) — is owned by Prompt Engineering & Governance §15.1 and is not redefined here.
