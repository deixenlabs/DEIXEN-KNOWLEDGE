...
| Pass | Status | Basis |
|---|---|---|
| A — Basic Foundations | PARTIALLY COMPLETE, mature in places | A1–A4 plus incidental Basic-tier findings across D-series (D4, D5, D18 — D18 now RESOLVED) |
| B — Advanced Air | PARTIALLY COMPLETE | B1–B5 (B2 now RESOLVED; B5 now SUBSTANTIALLY RESOLVED pending acknowledgment) plus D6 (now RESOLVED), D10, D13, D15, D20 |
| C — Phase 1 Reconciliation | **RESOLVED THIS UPDATE — COMPLETE_WITH_DOCUMENTED_LIMITATIONS** (was: BLOCKED — 0% possible) | Four Phase 1 artifacts supplied and inspected — see `AeroBridge_PassC_Reconciliation_Report.md` |
| D — Workflow/State/Error/Recovery | **CLOSED WITH BOUNDED CARRY-FORWARD** | Per the dedicated closure report; not reopened here |
| E — Carrier/Airline Context | **BLOCKED — 0% possible** for its EgyptAir-specific half, unchanged | EG-1: actual curriculum document still not directly supplied (only cited by Phase 1). The general-vs-carrier-specific *classification exercise* itself (§37/38 of the constitution) has been applied incidentally wherever findings were logged (e.g., D1's OPW/OPC-vs-ADTK split), but a dedicated Pass E has not run |
...
| D6 (FXX) | Could the "historical correction" itself be the error rather than the new evidence? | **At the time this adversarial pass was written, genuinely undetermined.** *(Resolved at Pass C: four convergent sources, two from Phase 1's independent registry, confirm FXX is real. The prior "historical correction" now appears to have been the actual error — see the Canonical Pre-Opus Knowledge Reference, Carry-Forward Register PRICE-1.)* |
...
**Pass H conclusion:** no existing finding was overturned by this adversarial pass at the time it was written. Both D6 and D21 were correctly held as unresolved rather than falsely resolved at that point — the adversarial review confirmed caution was warranted, not excessive. *(Both were subsequently resolved/substantially clarified at Pass C, once the Phase 1 artifacts arrived — the caution held until real evidence closed the gap, which is the discipline working as intended, not being overturned.)* One structural bias (gap-finding emphasis) is named honestly rather than hidden.
...
| Pricing-family variants (FXX/FXY/FXL/TQT/TTE/FQF/FXG/FXQ/FXI) | MIXED, improved at Pass C | FXQ/FXI/FXY/FXG all confirmed via Phase 1; **FXX now RESOLVED** (was: conflicted, D6); FXL/TQT/TTE/FQF remain weakly evidenced, single-sourced, absent from Phase 1 | Scope decision still needed only for FXL/TQT/TTE/FQF — see §1.4 |
...
| Amadeus Offers | **SUBSTANTIALLY RESOLVED at Pass C — Cryptic `OFS` family, pending product-owner acknowledgment (was: CONFLICT_UNRESOLVED / ESCALATED)** | Phase 1 registry's `OFS`/`RTOF`/`TQQ`/`OFN`/`OFV`/`OFK` family, explicitly tied to Phase 1's citation of "the user's own stated 25-topic list" | Full independent confirmation still depends on this project directly inspecting EG-1 itself, not just Phase 1's citation of it |
...
D2, D3, D4, D10, D15, D19, B3 remain open, substantively unchanged. **D6, D18, B2 are now RESOLVED; B5 is SUBSTANTIALLY RESOLVED; D21 is confirmed at the domain-truth level and escalated as an implementation-side question rather than left as a live conflict** — see the Canonical Pre-Opus Knowledge Reference for the authoritative current register; this line is no longer treated as a live index and is retained only for provenance.
...
| D21 | Real function of `QE`/`QN`/`QD` vs. AeroBridge's implementation | **Confirmed at domain-truth level by three convergent lineages (Pass C); implementation-truth side still open — now the project's single highest-priority quarantine** | This project's two sources + Phase 1's registry + Phase 1's own prior internal audit, all converging | High — this now needs code re-inspection, not more research | HIGH (affects an already-shipped command family) | HIGH | Low for research; high for engineering | Route to Decision 7 / engineering for code re-inspection | Pass C complete; Engineering / Decision 7 | NO for research; YES for engine-reference reconciliation |
| D6 | FXX identity | **RESOLVED at Pass C — four convergent sources** | Two non-official sources (this project) + two independent official-adjacent sources (Phase 1: TAP Portugal training manual, Tourism School Guide) | Medium — the prior curriculum "historical correction" should be formally revisited | LOW (not in frozen slice) | LOW | Low — verify under Opus review | None — a documentation-retraction action, not research | Product owner / curriculum documentation | Formal retraction of the prior correction |
...
All of 5.1, **plus**: workflow position known or explicitly marked UNKNOWN (never omitted silently) · failure/recovery behavior evidenced or explicitly labeled unverified · Basic/Advanced placement evidenced from curriculum comparison, not inferred from complexity · scope (core vs. extension vs. out-of-scope) explicitly assigned · **no item currently flagged UNVERIFIED, CONFLICT_UNRESOLVED, or NEEDS_EXTERNAL_CONFIRMATION may be presented as settled.** **Updated post-Pass-C:** D6 and D18 are now RESOLVED and no longer barred; B2 is now RESOLVED; B5 is SUBSTANTIALLY RESOLVED and may be used with its qualifying confidence statement attached (see the Canonical Pre-Opus Knowledge Reference §H-6), still short of unqualified promotion. **Still barred:** D19 (as Cryptic fact), D21 (as either side "winning" — see the Canonical Reference §H-2, now the single highest-priority quarantine in the project), B3, and voiding (§H-1, one targeted check performed, still unconfirmed).
...
**Standing rule, restated because it is the single most important sentence in this deliverable:** high-impact learner-facing knowledge must not be promoted while critical evidence gaps remain. **Updated post-Pass-C:** D6, D18, and B2 have cleared this bar and are no longer barred. B5 has substantially cleared it, carried forward with its qualifying confidence statement. **Still currently barred from learner-facing promotion: D19 (as Cryptic fact), D21 (queue command family — now the project's single highest-priority quarantine, since it involves already-shipped behavior, not merely prospective content), voiding, and Seat/SSR workflow detail.** This is not a defect in the research, it is the acceptance-criteria system working as designed — and its having actually moved several items across the bar this update is itself evidence it is not merely decorative.
...
| Check | Result |
|---|---|
| Contradictory claims across documents | None found once synchronized. D6 and D21 were held open consistently across documents *while they were open*; both are now updated consistently to their post-Pass-C status everywhere they appear (this document, the Research Log, the Canonical Pre-Opus Knowledge Reference) |
