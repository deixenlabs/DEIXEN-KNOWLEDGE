...
| `design/phase4/` — 68 `*.dc.html` boards, `tokens.css`, `MANIFEST.md` | Read-only copy of the approved Phase 4 design for Claude Code (07 D45) — owns nothing: the canvas (07 D42) is the original, and a file that fails its manifest sha256 is not the approved design | Approved by Karim 2026-09-26 (07 D45); copied unchanged from canvas version `1790413092-0523` |
| `CLAUDE.md` | Claude Code instructions, repository layout, verification checklist (Execution Plan 3.5) | Approved 2026-09-25 (07 D26), D27 applied; D30/D33/D34 applied to §3; 07 D42 applied to §2/§4/§5/§8, D45 to §2/§4 (2026-09-26) — master copy; moves into the code repository `deixen-app` at build start |
...
| `DEIXEN_MASTER_PRE_OPUS_FOUNDATION_PREPARATION_PLAN-1.md` | Ended by Karim, 2026-09-24. Not in the repository (checked 2026-09-26; earlier versions of this map said it was) |
| `DEIXEN_MASTER_EXECUTION_ROADMAP.md` | Replaced by the Execution Plan. Not in the repository (checked 2026-09-26) |
| Repository folders `Historical/` and `Historical /` (trailing space) | Found 2026-09-26: the OLD-generation documents (`PROJECT-20.md`, `SDD.md`, `AMADEUS_CURRICULUM.md`, `COMMAND_REFERENCE.md`, `DESIGN_SYSTEM_UI_BLUEPRINT.md`, `DEVELOPMENT_RULES.md`, `PRODUCT_STRATEGY_UX_ARCHITECTURE-1.md`), the five NEW-generation `AeroBridge_*` source documents and the consolidation prompt, the multi-model audit files, and the Approved Corpus Review & Opus Readiness Brief. All historical (07 D16); no current file depends on them. Earlier versions of A9 listed several of them as absent |
...
`11_…Consistency_Audit_and_Readiness_Review`; the Claude/Manus arbitration
record; the Domain/SME Validation Brief and SME-01…SME-10 register; the
Scenario Bank architecture review; the Learning Design adversarial review;
the Phase 1 registry files
...
the EgyptAir curriculum document; the two historical plans named in A8; and
every codebase (the vanilla-JS `aerobridge-main.zip` and the React "Rebound
Baseline").

*Correction 2026-09-26 (repository check):* the Approved Corpus Review &
Opus Readiness Brief and the OLD- and NEW-generation source documents named
in Part B (`PROJECT-20.md`, `SDD.md`, `AMADEUS_CURRICULUM.md`,
`COMMAND_REFERENCE.md` and the others) are **not** absent — they are in the
repository's historical folders (A8). They stay historical (07 D16); this
changes no decision and no current file.
...
repository also holds historical folders not listed before (A8, A9
correction).
...
the code repository (`deixen-app/src/styles/tokens.css`, CLAUDE.md §5); a
read-only copy of those boards is in `design/phase4/` (07 D45). The
