---
name: DEIXEN (Working Name) — AI Operating Environment — Final Pre-Opus Reconciliation
...
## Current Identity Note — Provisional Working Name

The project is currently being referred to as **DEIXEN** as a **Working Name / Provisional Project Identity**. This is a current operating label, not a final brand, legal, trademark, or permanent identity decision.

The name itself remains **subject to experimentation, evaluation, and possible replacement**. Its use in this document does not establish that it is final, approved, or canonical. Any later naming decision should be based on evidence, fit with project intent, usability, identity coherence, and relevant external or practical validation.

References to **AeroBridge** in this document are retained where they reflect the historical identity, existing artifact/repository/file names, provenance, or prior-stage material. Such references should not be read as a current decision to retain AeroBridge as the project's working name.

# DEIXEN (Working Name) — AI Operating Environment: Final Pre-Opus Reconciliation
...
### B.1A — Independent Reassessment Principle

The clean-context Opus Foundation Review must evaluate the prepared project independently rather than preserve prior decisions merely because they already exist.

Opus should:

preserve elements that remain well-supported, coherent, and fit for purpose;

strengthen elements where a materially better formulation, structure, or solution is justified;

challenge weakly supported, unresolved, or internally inconsistent assumptions;

recommend replacement or removal of elements that are materially defective, contradictory, or demonstrably harmful to the project's current or long-term interests;

preserve uncertainty where evidence is insufficient rather than manufacture closure.


Opus must not redesign, replace, or recommend removal merely because it prefers an alternative. Any material recommendation for change must be supported by evidence, explicit reasoning, alignment with project intent, consideration of downstream impact, and consideration of long-term consequences.

The report preceding the Opus review is methodological context and prior analysis, not substantive authority. Opus must form its corpus-level findings from the current project evidence and must not treat this report's conclusions as established truth merely because they appear here.

Final acceptance, rejection, and canonicalization remain subject to the project's established human decision authority.
...
### B.12A — Evidence Boundary: Design, Implementation, and Learning Effectiveness

**ADD — Strong candidate / operating principle, not canonical product or learning truth.**

The operating environment must keep three distinct forms of validation separate:

1. **Design / Documentation Validity**  
   Whether the approved product, learning, experience, and design architecture is coherent, internally consistent, sufficiently justified, and appropriately grounded in evidence.

2. **Implementation Validity**  
   Whether the implemented system faithfully and correctly realizes the approved behavior, state transitions, interaction contracts, evidence/state rules, and defined domain boundaries. Implementation correctness must not be mistaken for real-world domain truth; domain-sensitive claims remain subject to the project's established validation path.

3. **Empirical Learning Effectiveness**  
   Whether real learners actually become more capable through use of DEIXEN, as demonstrated by appropriate evidence from real learner use and evaluation.

These forms of validation are related, but they are not interchangeable.

Strong documentation does not establish correct implementation.
Correct implementation does not establish learning effectiveness.
Visual polish does not establish either.
AI confidence, model agreement, or a successful review pass does not substitute for the evidence required at any of these levels.

The evidence required for each level should match the claim being made:

- Design / Documentation Validity may be assessed through document review, cross-document consistency checks, evidence review, and independent adversarial review.
- Implementation Validity should be assessed through implementation inspection, behavioral testing, integration/regression testing, and domain validation wherever the implementation depends on real-world domain truth.
- Empirical Learning Effectiveness requires evidence from actual learner use and an appropriate evaluation method; it must not be inferred from architecture quality, feature completeness, implementation correctness, user-interface polish, or AI judgment alone.

Accordingly, no AI agent, review pass, prototype, implementation milestone, or documentation state may imply or represent empirical learning effectiveness as established unless the relevant real-learner evidence actually exists.

This boundary is an evidence-discipline rule for the operating environment. It does not create a new source of product, learning, domain, or design truth, and it does not override the ownership of the canonical corpus. Its purpose is to prevent different kinds of “success” from being conflated during review, implementation, or future claims about DEIXEN.
