...
The depth of any given handoff should scale with the session's materiality — proximity to a gate or decision point, how many items remain genuinely unresolved, how much downstream work depends on this state, and the cost of being misunderstood — not a fixed rule applied regardless of stakes. A routine session inside an already-stable phase may need only Sections 5, 6, 30, 32, and 43 populated; a phase- or gate-transition handoff needs the full template below. This is a judgment call proportional to risk, not a mandatory checklist.
...
1.1 Consultative Input Principle (adapted from 08 §8 for a receiving system that may not yet have a role assigned)
...
10. Do not treat a recommendation, review finding, execution result, or session conclusion as an approved decision merely because it appears in a newer handoff.
...
Where currentness is material to the task and a live source is accessible, verify the handoff's stated artifact version/revision against that live source before treating the Source-of-Truth Inventory (§9) as current.
...
10.1 Epistemic / Claim Status (per Prompt Engineering & Governance §29)
...
Work-Task Classification (per Prompt Engineering & Governance §15.1):
...
A queued action is a recommended next step, not a grant of execution authorization. Before acting on any entry here, the receiving system confirms current authorization per §30's Execution-Authorization Mode — the property listed above is what the writing session should already have checked, not a standing license for whichever session executes it.
...
A new field, register, or ID namespace in this template earns its place only by preventing a demonstrated or clearly foreseeable loss of continuity — not by appearing thorough. Extend this template with the same restraint 08 §32 and Prompt Engineering & Governance §71 apply to themselves.
